...
const EmployeeFormPanel: React.FC<{
  isOpen: boolean;
  employee: any;
  editType: 'general' | 'salary';
  onClose: () => void;
  onSubmit: (data: any) => Promise<void>;
}> = ({ isOpen, employee, editType, onClose, onSubmit }) => (
  <div className='absolute top-0 right-0 w-1/2 min-w-[600px] h-full bg-white border-l border-gray-200 flex flex-col overflow-hidden shadow-2xl transform transition-transform duration-300 ease-in-out z-10'>
    <div className='flex items-center justify-between p-4 border-b border-gray-200 bg-white'>
      <h2 className='text-lg font-semibold text-gray-900'>Edit Employee - General Information</h2>
    ...
    <div className='flex-1 overflow-auto'>
      <EmployeeForm
        ...
        isOpen={isOpen}
        isEdit={true}
        employeeData={{ ...employee, email: employee.email ?? '' }}
        editType={editType}
        onClose={onClose}
  onSubmit={onSubmit}
      />
